...
namespace RMS
{
    public partial class Rms_Set_Limit : Form
    {
        string str;
        public Rms_Set_Limit()
        {
            InitializeComponent();
        }
        ...
        void search()
        {
            ...
            doc.Load(s + "\\Rms_set_Limit.xml");
            XmlNodeList nodeList = doc.SelectNodes("//StudentDetailss/Student[BranchId='" + comboBox_Branch_id.SelectedItem.ToString() + "']");

            foreach (XmlNode node in nodeList)
            {
                if (comboBox_Branch_id.SelectedItem.ToString() == (node.SelectSingleNode("BranchId").InnerText))
                {
                    ...
                    XmlNodeList nodeListtt = doccc.SelectNodes("//StudentDetailss/Student[EntityId='" + textBox_Entity_id.Text + "']");
                        ...
                        foreach (XmlNode nodee in node)
                        {
                            ...
                            if (textBox_Entity_id.Text == (nodee.SelectSingleNode("EntityId").InnerText))
                            {
                                str = (nodee.SelectSingleNode("EntityId").InnerText);
                                break;
                            }
                   ...
                   // str = (node.SelectSingleNode("EntityId").InnerText);
                    //if (textBox_Entity_id.Text == (node.SelectSingleNode("EntityId").InnerText))
                        if (textBox_Entity_id.Text == str)
                     
                    {
                       
                        XmlDocument docc = new XmlDocument();
                        docc.Load(s + "\\Rms_set_Limit.xml");
                        XmlNodeList nodeListt = docc.SelectNodes("//StudentDetailss/Student[EntityId='" + textBox_Entity_id.Text + "']");
                        //if (textBox_Entity_id.Text == (node.SelectSingleNode("EntityId").InnerText))
                        //{

                        textBox_Cash_Fund.Text = (node.SelectSingleNode("CashFund").InnerText);

                        textBox_M_TO_M_Limit.Text = (node.SelectSingleNode("MTOMLimit").InnerText);

                        textBox_Additional_Fund.Text = (node.SelectSingleNode("AdditionalFund").InnerText);

                        textBox_Exporsure_Limit.Text = (node.SelectSingleNode("ExporsureLimit").InnerText);


                        textBox_Total_Buy_Qty_Limit.Text = (node.SelectSingleNode("TotalBuyQtyLimit").InnerText);

                        textBox_Turnover_Limit.Text = (node.SelectSingleNode("TurnoverLimit").InnerText);


                        textBox_total_sell_qty_limit.Text = (node.SelectSingleNode("TotalSellQtyLimit").InnerText);

                        textBox_portfolio_limit.Text = (node.SelectSingleNode("PortfolioLimit").InnerText);

                        textBox_single_order_qty_limit.Text = (node.SelectSingleNode("SingleOrderQtyLimit").InnerText);

                        textBox1_Max_Qty_Per_Portfolio.Text = (node.SelectSingleNode("MaxQtyPerPortfolio").InnerText);

                        textBox_single_order_value_limit.Text = (node.SelectSingleNode("SingleOrderValueLimit").InnerText);

                        textBox1_O_S_Position_Limit.Text = (node.SelectSingleNode("OSPositionLimit").InnerText);
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Branch Id And Entity Id Not Match");
                        textBox_Cash_Fund.Text = "0.00";
                        textBox_M_TO_M_Limit.Text = "0.00";
                        textBox_Additional_Fund.Text = "0.00";
                        textBox_Exporsure_Limit.Text = "0.00";
                        textBox_Total_Buy_Qty_Limit.Text = "0.00";
                        textBox_Turnover_Limit.Text = "0.00";
                        textBox_total_sell_qty_limit.Text = "0.00";
                        textBox_portfolio_limit.Text = "0.00";
                        textBox_single_order_qty_limit.Text = "0.00";
                        textBox1_Max_Qty_Per_Portfolio.Text = "0.00";
                        textBox_single_order_value_limit.Text = "0.00";
                        textBox1_O_S_Position_Limit.Text = "0.00";
